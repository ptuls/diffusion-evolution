...
def make_plot(
    ...
    focus_id=20,
    row=0,
    plot_diffusion=plot_diffusion_two_peaks,
    draw_circle=False,
):
    ...
    for i, t in enumerate(time_steps):
        ...
        if draw_circle:
            circle = plt.Circle(
                [-1, -1], 0.5, color="black", fill=False, zorder=2, linestyle="--", alpha=0.5
            )
...
def plot_distance_histogram(
    ...
    plt.hist(
        project_to_1d(x0_trace[t]).numpy(),
        bins=32,
        density=True,
        alpha=0.75,
        range=(-3, 3),
    ...
    plt.hist(
        project_to_1d(population_trace[t]).numpy(),
        bins=32,
        density=True,
        alpha=0.5,
        range=(-3, 3),
...
def make_plot_distance_histogram(x0_trace, population_trace, row=0, total_step=100):
    ...
    for i, t in enumerate(time_steps):
        ...
        plot_distance_histogram(
            x0_trace, population_trace, t, total_step=total_step, label=(i == 3), ylabel=(i == 0)
        )
...
if __name__ == "__main__":
    ...
    result_two_peak, x0_trace, scheduler_two_peak = optimizer(
