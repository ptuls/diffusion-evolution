...
def plot_diffusion(alpha_t, trace, fitnesses, focus_id, T, num_sample=100, dt=25, past_ts=[]):
    ...
    plt.plot(
        trace[:next_t, focus_id, 0], trace[:next_t, focus_id, 1], "-", color="#F5851E", zorder=3
    )
    ...
    u = v / torch.norm(v)
    plt.arrow(
        selected_p[0] + 0.2 * u[0],
        selected_p[1] + 0.2 * u[1],
        v[0] - 0.4 * u[0],
        v[1] - 0.4 * u[1],
        head_width=0.1,
        head_length=0.1,
...
def make_plot(optimizer, trace, fitnesses, method: str, time_steps=[20, 45, 70, 95]):
    ...
    for i, t in enumerate(time_steps):
        ...
        plot_diffusion(
            alpha_t, trace, fitnesses, focus_id=20, T=t, num_sample=100, dt=23, past_ts=past_ts
        )
