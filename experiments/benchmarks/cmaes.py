...
def CMAES_experiment(obj, num_steps=10, sigma_init=1):
    es = CMAES(
        num_params=2, popsize=512, sigma_init=sigma_init, weight_decay=1e-3, inopts={"seed": np.nan}
...
def prepare_data(obj, trace, arg, fitnesses):
    info = {"arguments": arg, "trace": trace, "fitnesses": fitnesses}
    return info
...
def CMAES_benchmark(
    ...
    **kwargs
    ...
    arg = {"num_step": num_steps, "sigma_init": sigma_init, "limit_val": limit_val}
    ...
    for i, foo_name in enumerate(objs):
        ...
        es, mu, cor, trace, fitnesses = CMAES_experiment(
            obj_rescaled, num_steps=num_steps, sigma_init=sigma_init
        )
...
if __name__ == "__main__":
    ...
    record = CMAES_benchmark(
        objs, num_steps=10, row=0, total_row=1, limit_val=100, plot=True, sigma_init=4
    )
