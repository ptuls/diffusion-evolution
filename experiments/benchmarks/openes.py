...
def OpenES_plot(obj, es, traj, mu, ax=None, traces=32):
    ...
    pop = traj[-1]
    plt.scatter(
        pop[:, 0], pop[:, 1], c=x0_color, alpha=0.5, marker=".", zorder=10, edgecolors="none"
    )
...
def OpenES_benchmark(
    objs, num_steps, row=0, total_row=4, total_col=5, sigma_init=4, plot=False, **kwargs
):
    arg = {"num_step": num_steps, "sigma_init": sigma_init}
    ...
    for i, foo_name in enumerate(objs):
        ...
        es, traj, fitnesses, mu = OpenES_experiment(
