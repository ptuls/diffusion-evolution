...
def cartpole_plot(
    ...
    plt.plot(
        [x0[i] - cart_size, x0[i] + cart_size], [y0[i], y0[i]], color=color, alpha=alpha * max_alpha
    )
...
def plot_cartpole(
    ...
    if color_bar:
        ...
        cbar = ax.figure.colorbar(
            plt.cm.ScalarMappable(cmap=custom_cmap), cax=cbar_ax, orientation="horizontal"
        )
...
def reward_compare_plot(*rewards, labels=None, colors=None, ax=None):
    ...
    ax.axhline(500, color="gray", linestyle="--", label="max reward", alpha=0.5)
...
def compare_latent_plot(pop, pop_raw, pop_cmaes, random_map, pop_large, random_map_large, ax=None):
    if ax is None:
        ax = plt.gca()
    latent_plot(
        random_map(pop).detach()[-1],
        ax,
        color="#E93A01",
        label="latent diffusion evolution",
        alpha=0.5,
    )
    latent_plot(
        random_map(pop_raw).detach()[-1],
        ax,
        color="#46B3D5",
        alpha=0.1,
        label="diffusion evolution",
    )
    latent_plot(random_map(pop_cmaes).detach()[-1], ax, color="#6F6E6E", alpha=0.5, label="CMA-ES")
    latent_plot(
        random_map_large(pop_large).detach()[-1],
        ax,
        color="#F5851E",
        alpha=0.25,
        label="latent DiffEvo (high-d)",
    )
    ...
    latent_line = Line2D(
        ...
        linestyle="None",
        markersize=10,
        label="latent DiffEvo",
    )
    raw_line = Line2D(
        [0], [0], marker=".", color="#46B3D5", linestyle="None", markersize=10, label="DiffEvo"
    )
    cmaes_line = Line2D(
        [0], [0], marker=".", color="#6F6E6E", linestyle="None", markersize=10, label="CMA-ES"
    )
    large_line = Line2D(
        [0],
        [0],
...
if __name__ == "__main__":
    ...
    reward_compare_plot(
        ...
        colors=["#46B3D5", "#E93A01", "#F5851E", "#6F6E6E"],
        ax=ax2,
    )
    ...
    compare_latent_plot(
        pop_latent, pop_raw, pop_cmaes, random_map, pop_large, random_map_large, ax=ax3
    )
